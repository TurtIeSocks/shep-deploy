...
A cutover that was abandoned leaves the tree behind, and **a sheep is not a deploy target until a cutover lands.** Its record names no deployed release, so `shep deploy` against it does not stop: it builds, swaps, reloads the sheep at its own checkout, sees a real turnover, and reports success for a release nothing served. Remove the tree under `$SHEP_HOME/deploy/<sheep>` and run `setup` again once the cause is fixed. Both `setup` and the failure message say so, by name.
